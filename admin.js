...
function loadDashboardData() {
  ...
  document.getElementById('active-today').textContent = todayActiveSessions;
...
function searchSessions(query) {
  ...
  const sortedSessions = Object.entries(filteredSessions).sort((a, b) => {
    const aLastMsg = a[1].messages.length > 0 ? new Date(a[1].messages[a[1].messages.length - 1].time) : new Date(0);
    const bLastMsg = b[1].messages.length > 0 ? new Date(b[1].messages[b[1].messages.

// Search sessions (continued)
function searchSessions(query) {
  if (!query) {
    loadChatSessions();
    return;
...
function saveSettings() {
  try {
    const settings = {
      ...
      showTypingIndicator: document.getElementById('show-typing-indicator').checked,
      ...
      formspreeEndpoint: document.getElementById('formspree-endpoint').value,
      saveToServer: document.getElementById('save-to-server').checked,
